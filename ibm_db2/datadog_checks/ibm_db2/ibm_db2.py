...
class IbmDb2Check(AgentCheck):
    ...
    def query_database(self, connection, tags):
        ...
        if result['last_backup']:
            seconds_since_last_backup = (result['current_time'] - result['last_backup']).total_seconds()
        else:
    ...
    def query_buffer_pool(self, connection, tags):
        ...
        while bp:
            ...
            self.monotonic_count(self.m('buffer.column.reads.physical'), column_reads_physical, tags=bp_tags)
            ...
            self.monotonic_count(self.m('buffer.column.reads.logical'), column_reads_logical, tags=bp_tags)

            # Submit total
            self.monotonic_count(
                self.m('buffer.column.reads.total'), column_reads_physical + column_reads_logical, tags=bp_tags
            )
            ...
            self.gauge(self.m('buffer.column.hit_percent'), column_hit_percent, tags=bp_tags)
            ...
            if group_column_reads_logical:
                ...
                self.gauge(self.m('buffer.group.column.hit_percent'), group_column_hit_percent, tags=bp_tags)
            ...
            self.monotonic_count(self.m('buffer.data.reads.physical'), data_reads_physical, tags=bp_tags)
            ...
            self.monotonic_count(self.m('buffer.data.reads.logical'), data_reads_logical, tags=bp_tags)

            # Submit total
            self.monotonic_count(
                self.m('buffer.data.reads.total'), data_reads_physical + data_reads_logical, tags=bp_tags
            )
            ...
            if data_reads_logical:
                ...
                data_hit_percent = 0
            self.gauge(self.m('buffer.data.hit_percent'), data_hit_percent, tags=bp_tags)
            ...
            if group_data_reads_logical:
                ...
                self.gauge(self.m('buffer.group.data.hit_percent'), group_data_hit_percent, tags=bp_tags)
            ...
            self.monotonic_count(self.m('buffer.index.reads.physical'), index_reads_physical, tags=bp_tags)
            ...
            self.monotonic_count(self.m('buffer.index.reads.logical'), index_reads_logical, tags=bp_tags)

            # Submit total
            self.monotonic_count(
                self.m('buffer.index.reads.total'), index_reads_physical + index_reads_logical, tags=bp_tags
            )
            ...
            if index_reads_logical:
                ...
                index_hit_percent = 0
            self.gauge(self.m('buffer.index.hit_percent'), index_hit_percent, tags=bp_tags)
            ...
            if group_index_reads_logical:
                ...
                self.gauge(self.m('buffer.group.index.hit_percent'), group_index_hit_percent, tags=bp_tags)
            ...
            self.monotonic_count(self.m('buffer.xda.reads.physical'), xda_reads_physical, tags=bp_tags)
            ...
            self.monotonic_count(self.m('buffer.xda.reads.logical'), xda_reads_logical, tags=bp_tags)

            # Submit total
            self.monotonic_count(
                self.m('buffer.xda.reads.total'), xda_reads_physical + xda_reads_logical, tags=bp_tags
            )
            ...
            if xda_reads_logical:
                ...
                xda_hit_percent = 0
            self.gauge(self.m('buffer.xda.hit_percent'), xda_hit_percent, tags=bp_tags)
            ...
            if group_xda_reads_logical:
                ...
                self.gauge(self.m('buffer.group.xda.hit_percent'), group_xda_hit_percent, tags=bp_tags)

            # Compute overall stats
            reads_physical = column_reads_physical + data_reads_physical + index_reads_physical + xda_reads_physical
            self.monotonic_count(self.m('buffer.reads.physical'), reads_physical, tags=bp_tags)

            reads_logical = column_reads_logical + data_reads_logical + index_reads_logical + xda_reads_logical
            self.monotonic_count(self.m('buffer.reads.logical'), reads_logical, tags=bp_tags)

            reads_total = reads_physical + reads_logical
            self.monotonic_count(self.m('buffer.reads.total'), reads_total, tags=bp_tags)
            ...
            if reads_logical:
                ...
                hit_percent = 0
            self.gauge(self.m('buffer.hit_percent'), hit_percent, tags=bp_tags)
            ...
            if group_reads_logical:
                ...
                self.gauge(self.m('buffer.group.hit_percent'), group_hit_percent, tags=bp_tags)
